...
library(ggplot2)
StrataMax<-6
Enduses<-c("Results","CustomElectric", "Motor", "CompressedAirEquip", "HVAC","T8","OtherLighting","OtherElectric","LED")
...
CEopt <- CE[CE$PrimaryMeasure==Measure ,c(1,10,12,16)]
...
Results<-matrix(data = NA,nrow = (Strata+1),ncol=length(Enduses))
Results[1,]<-Enduses
for (r in 1:Strata){
...
system.time(for (i in 1:Strata){
  Results[1,(Measure==Results[1,])]<-Measure
  Results[i+1,(Measure==Results[1,])]<-min(CVsCount[(CVsCount[,(length(CEopt$Work))]==i),(length(CEopt$Work)+2)],na.rm = TRUE)
})
