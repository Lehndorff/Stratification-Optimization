...
Results<-matrix(data=NA, nrow=(Strata*length(Endusesn)), ncol = (StrataMax+5))
colnames(Results)<-c("Enduse","Strata","Iter", "BestCV","S1","S2","S3","S4","S5","S6", "Max")
Result<-0
# h<-2
system.time(
  for (h in Endusesn){
...
# CE$Percent <- CE$SumKWH/CE$kWh_sum
