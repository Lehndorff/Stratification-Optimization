...
TrendpUp$YES<-0
TrendpUp<-as.data.frame(TrendpUp%>%group_by(V1)%>%mutate(min=min(V2,V3,V4,V5,V6,V7,V8,V9,V10,V11,na.rm=TRUE),max=max(V2,V3,V4,V5,V6,V7,V8,V9,V10,V11,na.rm=TRUE)))
TrendpUp$mean<-TrendpUp$sum/length(Marks)
TrendpUp$minfirst<-as.numeric(TrendpUp$min==TrendpUp$V11)
TrendpUp$maxlast<-as.numeric(TrendpUp$max==TrendpUp$V2)
# TrendpUp$YES[TrendpUp$V2>.6&TrendpUp$V3>.6&TrendpUp$V4>.6&TrendpUp$V5>.6&TrendpUp$V6>.55]<-1
for (t in 1:length(TrendpUp$V1)){
  TrendpUp$YES[t]<-((sum(TrendpUp[t,2:5]>=.6,na.rm = TRUE)+sum(TrendpUp[t,6:9]>.55,na.rm = TRUE))+sum(TrendpUp[t,10:11]>.52,na.rm = TRUE))*
    (sum(TrendpUp[t,2:11]<=.5,na.rm=TRUE)==0)*(sum(TrendpUp$minfirst[t],TrendpUp$maxlast[t],na.rm = TRUE)>=1)*(sum(TrendpUp$V2[t],na.rm = TRUE)>=.6)
}
# for (t in 1:length(TrendpUp$V1)){
...
symbolsWatch<-as.vector(unique(UpX$V1[((UpX$YES>=8)+(UpX$`TrendpScore$sum`>=.2)+(abs(UpX$V2)>=8))==3]))
...
TrendupWatch2<-as.data.frame(TrendupWatch)
FinalWatch<-Final[Final$V1 %in% symbolsWatch,]

Tomorrow<-matrix(data = NA, nrow=length(symbols),ncol = 3)
Tomorrow[,1]<-symbols
for (j in 1:length(symbols)){
  STOCK2<-na.approx(get(symbols[j]))
  colnames(STOCK2)<-c("open","high","low","close","volume","adjusted")
  Tomorrow[j,2]<-((STOCK2$close-lag(STOCK2$close))/lag(STOCK2$close))[nrow(STOCK2)-bit+2]
  Tomorrow[j,3]<-STOCK2$close[nrow(STOCK2)-bit+1]
}
